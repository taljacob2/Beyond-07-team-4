...
def create_users(db):
    ...
    assert User.objects.filter(username='user0').first().exists()
    user1 = User.objects.create_user('user1', password='123345')
    assert User.objects.filter(username='user1').first().exists()
    user2 = User.objects.create_user('user2', password='123678')
    assert User.objects.filter(username='user2').first().exists()
    user3 = User.objects.create_user('user3', password='123321')
    assert User.objects.filter(username='user3').first().exists()
